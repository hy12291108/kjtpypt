...
public class CmsUtils {
	...
	public static List<Article> getArticleListByXxyd(String siteId, String categoryId, int number, String param){
		...
		article.setDelFlag(Article.DEL_FLAG_NORMAL);
		article.setAttachmentName("1");
		page = articleService.findPagexxyd(page, article, false);
		return page.getList();
	}
	/**
	 * 查询首页显示交流列表
	 * @param number 数量
	 * @return
	 * ${fnc:getCommunicationList(number)}"
	 * 20171102
	 */
	public static List<Communication> getCommunicationList(int number){
		Page<Communication> page = new Page<Communication>(1, number, -1);
		Communication communication = new Communication();
		communication.setDelFlag(Article.DEL_FLAG_NORMAL);
