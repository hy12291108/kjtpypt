...
public class ArticleService extends CrudService<ArticleDao, Article> {
	 ...
	 */
	@Transactional(readOnly = false)
	public Article getZjzx(String id){
		return articleDao.getzjzxArticle(id);
	}
	@Transactional(readOnly=false)
...
//		System.out.println(queryFilter);
	...
	//保存学习园地信息
	@Transactional(readOnly = false)
	public void savexxyd(Article article) {
		if (article.getArticleData().getContent()!=null){
			article.getArticleData().setContent(StringEscapeUtils.unescapeHtml4(
					article.getArticleData().getContent()));
		}
		...
		// 如果栏目不需要审核，则将该内容设为发布状态
		if (article.getCategory()!=null&&StringUtils.isNotBlank(article.getCategory().getId())){
			Category category = categoryDao.getxxyd(article.getCategory().getId());
			if (!Global.YES.equals(category.getIsAudit())){
				article.setDelFlag(Article.DEL_FLAG_NORMAL);
			}
		}
	 ...
	 */
	@Transactional(readOnly = false)
	public void savezjzx(Article article,HttpSession session) {
		if (article.getArticleData().getContent()!=null){
			article.getArticleData().setContent(StringEscapeUtils.unescapeHtml4(
					article.getArticleData().getContent()));
		}
	...
	//学习园地列表
	@Transactional(readOnly = false)
	public Page<Article> findPagexxyd(Page<Article> page, Article article, boolean isDataScopeFilter) {
		// 更新过期的权重，间隔为“6”个小时
		Date updateExpiredWeightDate =  (Date)CacheUtils.get("updateExpiredWeightDateByArticle");
		if (updateExpiredWeightDate == null || (updateExpiredWeightDate != null 
				&& updateExpiredWeightDate.getTime() < new Date().getTime())){
...
//		return dao.find(page, dc);
	 ...
	 */
	@Transactional(readOnly = false)
	public Page<Article> findPagezjzx(Page<Article> page, Article article, boolean isDataScopeFilter) {
		// 更新过期的权重，间隔为“6”个小时
		Date updateExpiredWeightDate =  (Date)CacheUtils.get("updateExpiredWeightDateByArticle");
		if (updateExpiredWeightDate == null || (updateExpiredWeightDate != null 
				&& updateExpiredWeightDate.getTime() < new Date().getTime())){
		...
		if (article.getCategory()!=null && StringUtils.isNotBlank(article.getCategory().getId()) && !Category.isRoot(article.getCategory().getId())){
			Category category = categoryDao.getzjzx("999");
			if (category==null){
				category = new Category();
			}
			category.setParentIds(category.getId());
		...
		article.setPage(page);
		page.setList(articleDao.findListzjzx(article));
		return page;
	}
	
	
	@Transactional(readOnly = false)
	public String upload(CommonsMultipartFile file,String attTable,Article article) throws IOException {
		//上传文件到服务器磁盘
...
//	    File localFile = new File(this.uploadePath + file.getOriginalFilename());
	...
	//获得附件信息
	@Transactional(readOnly = false)
	public List<SysAttachment> findAttatchments(String attachmentName) {
		SysAttachment sysAttachment=new SysAttachment();
		sysAttachment.setAttRecordid(attachmentName);
	 ...
	 */
	@Transactional(readOnly = false)
	public int updateZjzxSh(Article article){
		return articleDao.updateZjzxSh(article);
	}
}
