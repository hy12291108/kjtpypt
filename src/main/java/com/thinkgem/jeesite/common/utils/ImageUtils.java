...
public class ImageUtils {
     ...
     * @throws IOException
     */
    public static String upload(HttpServletRequest request, MultipartFile pictureFile, String ImageUrl) throws IOException {
        ...
        if (pictureFile != null && !pictureFile.isEmpty()) {
            ...
            String url = request.getSession().getServletContext()
                    .getRealPath(ImageUrl)+dataStr;
            // 检验文件夹是否存在
            isFolderExists(url);
            // 以绝对路径保存重名命后的图片
            pictureFile.transferTo(new File(url + name + "." + ext));
            // 装配图片地址
            imgPath = ImageUrl +dataStr + name + "." + ext;
        }
        return imgPath;
    }
