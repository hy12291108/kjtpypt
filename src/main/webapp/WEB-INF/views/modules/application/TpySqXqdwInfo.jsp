...
<body>	
    <ul class="nav nav-tabs">
		<li><a href="${ctx}/sqtpy/tpy/TpySqXqdwInfo">特派员申请帮扶</a></li>
	</ul>
	<c:if test="${fns:getUser().personFlag=='2'}">
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sqtpy/tpy/SaveTpySqXqdw" method="post" class="form-horizontal">
		<form:hidden path="id"/>
	...
	<c:if test="${fns:getUser().personFlag=='0'}">
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sqtpy/tpy/SaveTpySqXqdw" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<form:hidden path="personFlag"/>
		<sys:message content="${message}"/>	
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<form:input path="corpCorPhone"  class="required" readonly="true" htmlEscape="false" maxlength="50" />
				</td>		
			</tr>
