...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/sqtpy/tpy/Shlist");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sqtpy/tpy/Shlist">未审核列表</a></li>	
		<li ><a href="${ctx}/sqtpy/tpy/yshlist">已审核列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="sqtpy" action="${ctx}/sqtpy/tpy/Shlist" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="sqtpy">
			<tr>
				...
				<td><c:choose>				
						<c:when test="${sqtpy.state == '1'}">	
							<a href="${ctx}/sqtpy/tpy/selectShInfo?id=${sqtpy.id}&personFlag =${sqtpy.personFlag}"><font color=blue >审核</font> </a>					
						</c:when>
						<c:when test="${sqtpy.state == '2'}">
							<a href="${ctx}/sqtpy/tpy/selectShInfo1?id=${sqtpy.id}"><font color=blue >查看详情</font> </a>
						</c:when>
						<c:when test="${sqtpy.state == '3'}">
							<a href="${ctx}/sqtpy/tpy/selectShInfo1?id=${sqtpy.id}"><font color=blue >查看详情</font> </a>
						</c:when>
