...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/article/xxydform?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">学习园地文章<shiro:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/savexxyd" method="post" class="form-horizontal">
		<form:hidden path="id"/>
