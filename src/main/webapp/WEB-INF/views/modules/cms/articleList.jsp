...
<c:if test="${article.category.id != 999}">
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="cms:article:edit"><li><a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章添加
	...
	<c:if test="${article.category.id == 999}">
		<ul class="nav nav-tabs">
			...
			<!--  <li><a href="<c:url value='${fns:getAdminPath()}/cms/article/zjzxform?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">专家咨询添加</a></li>-->
		</ul>
		...
		<sys:message content="${message}"/>
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			<thead><tr><th>栏目</th><th>标题</th><th>点击数</th><th>发布者</th><th>更新时间</th><th>操作</th></tr></thead>
			<tbody>
			<c:forEach items="${page.list}" var="article">
				<tr>
					...
					<!--<c:choose>
		    				...
		    				<a href="${ctx}/cms/article/zjzxUpdate?id=${article.id}">修改</a>
