...
<head>
	...
	<script type="text/javascript">
		...
		function save(){
			...
			$("#files").attr("value", finallyPath); 
			$("#inputForm").attr("action", "${ctx}/cms/article/save");
			$("#inputForm").submit();
		}
		...
		function savezjzx(){
			...
			$("#files").attr("value", finallyPath); 
			$("#inputForm").attr("action", "${ctx}/cms/article/savezjzx");
			$("#inputForm").submit();
		}
	...
	<link href="${pageContext.request.contextPath}/static/common/qj.css" rel="stylesheet" type="text/css"/>
</head>
<body>
	<c:if test="${article.category.id != 999}">
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cms/article/?category.id=${article.category.id}">文章列表</a></li>
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章
		<shiro:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			<shiro:hasPermission name="cms:article:edit"><input id="btnSubmit" class="btn btn-primary" type="button" value="保 存" onclick="save()"/>&nbsp;</shiro:hasPermission>
	...
	<c:if test="${article.category.id == 999}">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/article/zjzxform?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">专家咨询添加</a></li>
		</ul><br/>
		<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/savezjzx" method="post" class="form-horizontal" enctype="multipart/form-data">
			<form:hidden path="id"/>
			<sys:message content="${message}"/>
			<div class="control-group">
				<label class="control-label">归属栏目:</label>
			...
			<div class="control-group">
				<label class="control-label">视频附件:</label>
				<div class="controls">
					<input   type="file" name="file" accept="audio/mpeg,audio/mp4,video/mp4" multiple="multiple"> <br/>
			...
			<div class="form-actions">
				<input id="btnSubmit" class="btn btn-primary" type="button" value="保 存" onclick="savezjzx()"/>&nbsp;
				<input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)"/>
			</div>
		</form:form>
	</c:if>
