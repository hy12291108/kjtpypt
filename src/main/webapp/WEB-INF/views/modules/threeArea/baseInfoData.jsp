...
<body>
	<ul id="ulWork" class="nav nav-tabs">
		<li class="active"><a href="${ctx}/threeSq/baseInfoData">三区申请参数维护</a></li>
		<li ><a href="${ctx}/threeSq/addBaseInfoData">三区申请参数添加</a></li>
	</ul>
	 <form action="${ctx}/threeSq/insertBaseData" method="post" class="form-inline">
	 	<input type="hidden" name="id" value="${threeAreaBaseData.id}"/>
		<table id="contentTable" class="table  table-bordered  ">
			<tbody>
				...
				<tr>
					<td>年度开始时间:</td>
					<td><input name="yearStartTime" placeholder="开始时间" id="d4311" class="Wdate" type="text" value="${threeAreaBaseData.yearStartTime}"  onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',isShowClear:false})" required/>
					<font color="red">*</font></span></td>
				</tr>
				<tr>
					<td>年度结束时间:</td>
					<td><input name="yearEndTime" placeholder="结束时间"  id="d4312" class="Wdate" type="text" value="${threeAreaBaseData.yearEndTime}" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',isShowClear:false,minDate:'#F{$dp.$D(\'d4311\')}'})" required/>
