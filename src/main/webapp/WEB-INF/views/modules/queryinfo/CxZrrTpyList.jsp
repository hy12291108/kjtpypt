...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/queryinfo/info/queryzrrtpyList">自然人特派员列表</a></li>
		<li><a href="${ctx}/queryinfo/info/queryfrtpyList">法人特派员列表</a></li>
	</ul>
		<form:form id="searchForm" modelAttribute="user" action="${ctx}/queryinfo/info/queryzrrtpyList" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>归属部门</th><th>姓名</th><th>性别</th><th>工作单位</th><th>专业</th><th>学历</th><th>职称</th><th>审核状态</th><th>操作</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="user">
			<tr>
				<td>${user.office.name}</td>
				<td>${user.name}</td>
				<td>${fns:getDictLabels(user.sex,'sex',user.sex)}</td>				
				<td>${user.tpyCompany}</td>
				<td>${user.tpyMajor}</td>
				<td>${user.tpyQulification}</td>
				<td>${user.tpyTitle}</td>					
				<td><c:choose>
