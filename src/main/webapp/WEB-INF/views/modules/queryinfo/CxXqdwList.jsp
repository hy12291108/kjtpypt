...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/queryinfo/info/queryxqdwList");
			$("#searchForm").submit();
	    	return false;
	    }
	      function corpList(){
...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/queryinfo/info/queryxqdwList">需求单位信息列表</a></li>
	</ul>
		<form:form id="searchForm" modelAttribute="user" action="${ctx}/queryinfo/info/queryxqdwList" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">			
			...
			<li><label>法&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;人：</label><form:input path="corpLegRepName" htmlEscape="false" maxlength="50" class="input-medium"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>归属部门</th><th>单位名称</th><th>单位类型</th><th>法人</th><th>联系方式</th><th>单位联系人</th><th>联系方式</th><th>注册时间</th><th>审核状态</th><th>操作</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="user">
			<tr>
				<td>${user.office.name}</td>
				<td>${user.name}</td>
				<td>${fns:getDictLabels(user.corpType,'corp_type',user.corpType)}</td>
				...
				<td>
    				<a href="${ctx}/queryinfo/info/queryxqdwInfo?id=${user.id}">查看</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
