...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/dailywork/tpy/tpyAnnualreport/");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/dailywork/tpy/tpyAnnualreport/">年度考核(待审核)</a></li>
	...
	<form:form id="searchForm" modelAttribute="tpyAnnualreport" action="${ctx}/dailywork/tpy/tpyAnnualreport/" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="tpyAnnualreport">
			<tr>
				...
				<c:choose>
					<c:when test="${tpyAnnualreport.repStatus == 'no'}">
							<a href="${ctx}/dailywork/tpy/tpyAnnualreport/change?id=${tpyAnnualreport.id}">提交</a>	
							<a href="${ctx}/dailywork/tpy/tpyAnnualreport/update?id=${tpyAnnualreport.id}">修改</a>
							<a href="${ctx}/dailywork/tpy/tpyAnnualreport/delete?id=${tpyAnnualreport.id}" onclick="return confirmx('确认要删除该特派员年度总结吗？', this.href)">删除</a>						 
						</c:when>
						<c:when test="${tpyAnnualreport.repStatus == 'inProcess'}">
