...
<head>
	...
	<link href="${ctxStatic}/bootstrap/2.3.1/css_default/style.css" type="text/css" rel="stylesheet"/>
	...
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			<div class="navbar-inner">
				...
				<ul id="userControl" class="nav pull-right">
					...
					<!-- 退出登录跳转修改2018-03-21 -->
					<!--<li><a href="http://www.sxkjtpy.cn/a" title="退出登录">退出</a></li>-->
						...
						<c:forEach items="${fns:getMenuList()}" var="menu" varStatus="idxStatus">
							<c:if test="${menu.parent.id eq '1'&&menu.isShow eq '1'}">
								...
								<%-- <c:if test="${empty menu.href}">
									...
									<c:if test="${menu.name=='系统设置'&&fns:getUser().id=='1'}">
										<a class="menu" href="javascript:" data-href="${ctx}/sys/menu/tree?parentId=${menu.id}" data-id="${menu.id}"><span>${menu.name}</span></a>
									</c:if>
									</c:if>
									<c:if test="${not empty menu.href}">
	    ...
	    <div class="container-fluid">
		    ...
		    <div id="footer" class="row-fluid">
	            Copyright &copy; 2017-${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a href="" target="_blank">西安协同数码股份有限公司</a> ${fns:getConfig('version')}
			</div>
		</div>
	</div>
	<script type="text/javascript"> 
