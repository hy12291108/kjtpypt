...
var uploadTools = {
    ...
    "initWithUpload": function (opt) {
        ...
        $("#" + uploadId + " .uploadBts .uploadFileBt").on("click", function () {
            uploadEvent.uploadFileEvent(opt, 1);
        });
    ...
    "addFileList": function (fileList, opt) {
        ...
        if (fileNumber + fileList.length > opt.maxFileNumber) {
            alert("最多只能上传" + opt.maxFileNumber + "张图片");
            return;
        }
     ...
     * 上传文件
     */
    "uploadFile": function (opt, number) {
        var uploadUrl = opt.uploadUrl;
        var fileList = uploadFileList.getFileList(opt);
        var formData = new FormData();
        var fileNumber = uploadTools.getFileNumber(opt);
        if (fileNumber <= 0) {
            top.$.jBox.tip('没有图片，不支持上传', 'warning');
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.fileList.length; i++) {
                        ...
                        //	fileList=fileList+data.fileList[i];
                        //}else{
                        fileList = fileList + data.fileList[i] + "|";
                        //}
                    }
                    if (number == 1) {
                        $("#tjTableImage").val(fileList);
                        $('#btnSubmit').removeAttr("disabled");
                        $('#btnCancel').removeAttr("disabled");
                        ...
                        //alert(fileList);
                        $("#drTableImage").val(fileList);
                        $('#btnSubmit').removeAttr("disabled");
                        $('#btnCancel').removeAttr("disabled");
                        $("#inputForm").attr("action", "/a/dailywork/tpy/tpyDailyrecord/save");
                        $("#inputForm").submit();
                    } else if (number == 3) {
                        $("#drTableImage").val(fileList);
                        $('#btnSubmit').removeAttr("disabled");
                        ...
                        $("#zjzxImage").val(fileList);
                        $('#btnSubmit').removeAttr("disabled");
                        $('#btnCancel').removeAttr("disabled");
                        $("#inputForm").attr("action", "/kjtpypt/a/cms/article/savezjzx1");
                        $("#inputForm").submit();
                    }else if (number == 5) {
                        $("#tjTableImage").val(fileList);
                        $('#btnSubmit').removeAttr("disabled");
                        $('#btnCancel').removeAttr("disabled");
                        $("#inputForm").attr("action", "/kjtpypt/a/UserSh/uploadTjTable");
                        $("#inputForm").submit();
                    }
...
var uploadEvent = {
     ...
     * 上传文件的事件
     * */
    "uploadFileEvent": function (opt, i) {
        opt.beforeUpload(opt);
        $("#btnSubmit").attr({"disabled": "disabled"});
        $("#btnCancel").attr({"disabled": "disabled"});
        uploadTools.uploadFile(opt, i);
    },
    /**
     * 清除文件事件
     */
